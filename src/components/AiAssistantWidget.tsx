...
export default function AiAssistantWidget({ currentAccount }: AiAssistantWidgetProps) {
  ...
  // Custom API key stored in localStorage so the AI continues working after push to GitHub
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error("API Key Gemini belum terkonfigurasi. Silakan klik ikon kunci di kanan atas untuk memasukkannya.");
      }
  ...
  // Render text and detect simple Markdown like bold and newlines
  const renderMessageText = (text: string) => {
    const parts = text.split(/(\*\*.*?\*\*)/g);
    return parts.map((part, index) => {
      if (part.startsWith("**") && part.endsWith("**")) {
        return <strong key={index} className="font-extrabold text-indigo-950">{part.slice(2, -2)}</strong>;
      }
      return part;
    });
  };
